...
class MixtureSameFamily(Distribution):
    ...
    has_rsample = False # No rsample (reparameterization samples) method in the base class

    def __init__(self,
                 mixture_distribution,
                 component_distribution,
                 validate_args=None):
        self._mixture_distribution = mixture_distribution # Categorical distribution
        self._component_distribution = component_distribution # Distribution for each component, e.g. Gaussian
        ...
        # the .batch_shape gives the dimensions of RVs ((RV1, RV2,...,RVn))
        # .event_shape gives the dimensions of the draw from the RV (d in MVN(mu_d, Sigma_d), mu_d in R^d)
        ...
        km = self._mixture_distribution.logits.shape[-1] # _mixture_distribution.batch is of [2], so use the .logits.shape
    ...
    def expand(self, batch_shape, _instance=None): # expand the batch_shape
        batch_shape = torch.Size(batch_shape)
        batch_shape_comp = batch_shape + (self._num_component,) # additional dimension for components (2, 4) + (d,) = (2, 4, d)
        new = self._get_checked_instance(MixtureSameFamily, _instance) # standard way to create a new instance for Distribution
    ...
    def mean(self):
        ...
        return torch.sum(probs * self.component_distribution.mean,
                         dim=-1 - self._event_ndims)  # [B, E], (batch_shape, event_shape)
    ...
    def cdf(self, x): 
        x = self._pad(x) # add a dimension before the event dimensions to match the distributions 
        cdf_x = self.component_distribution.cdf(x) # of shape [B, K]
        mix_prob = self.mixture_distribution.probs # of shape [B, K]

        return torch.sum(cdf_x * mix_prob, dim=-1) # sum over K

    def log_prob(self, x):
        if self._validate_args: # validate the variance
            self._validate_sample(x)
        x = self._pad(x)
        log_prob_x = self.component_distribution.log_prob(x)  # [S, B, k], S for samples shape
    ...
    def sample(self, sample_shape=torch.Size()):
        with torch.no_grad():
            sample_len = len(sample_shape) # length of the sample shape e.g., (5, ) -> 1
            batch_len = len(self.batch_shape) # the length of batch_shapes, e.g., [2] -> 1
            gather_dim = sample_len + batch_len # the position of dim for K (categorical part)
            es = self.event_shape # can be empty, if it is univariate

            # mixture samples [S, B]
            mix_sample = self.mixture_distribution.sample(sample_shape)
            mix_shape = mix_sample.shape # [S, B]

            # component samples [S, B, K, E]
            comp_samples = self.component_distribution.sample(sample_shape) # [S, B, K, E]

            # Gather (sample by torch.gather()) along the k dimension
            mix_sample_r = mix_sample.reshape(
                mix_shape + torch.Size([1] * (len(es) + 1))) # [S, B] -> [S, B, 1, (1, 1...)], number of (1, 1...) matches the dim of event_shape (es)
            mix_sample_r = mix_sample_r.repeat(
                torch.Size([1] * len(mix_shape)) + torch.Size([1]) + es) # repeat along the event dim, [S, B, 1, (E_1, E_2...)]
                # torch.Size([1]) is for the categorical dimension (K), es is for the event dimensions
            samples = torch.gather(comp_samples, gather_dim, mix_sample_r)
            return samples.squeeze(gather_dim) # squeeze the position for dimension of K

    def _pad(self, x):
        return x.unsqueeze(-1 - self._event_ndims) # add a dimension (K) before the event dimensions
...
class ReparametrizedMixtureSameFamily(MixtureSameFamily):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self._component_distribution.has_rsample: # Check if component distribution supports reparameterization (True)
    ...
    # This is the part of the code that implements Implicit Reparameterization Sampling
    def rsample(self, sample_shape=torch.Size()):
        ...
        if event_size != 1:
            ...
            transform = self._distributional_transform(x) # sum over the K, dim of [S, B]
            log_prob_x = self.log_prob(x)# shape of [S, B]
    ...
    def _distributional_transform(self, x):
        ...
        if isinstance(self._component_distribution, distr.Independent):
            univariate_components = self._component_distribution.base_dist # if multivariate (E > 1)
        else:
            univariate_components = self._component_distribution # if univariate (E == 1)
        ...
        if event_size != 1:
            ...
            log_posterior_weights_x = self._mixture_distribution.logits # un-normalized values
            posterior_weights_x = torch.softmax(log_posterior_weights_x, dim=-1) # shape of [2, 4] after normalization
...
if __name__ == "__main__":
    ...
    dims = (2, 4) # (batch, number of components)
    torch.manual_seed(1111)
    mixture_probs=torch.softmax(torch.randn(*dims), dim=-1) # of size [2, 4], mixture_probs[i].sum() = 1

    locs = torch.arange(prod(dims)).float().reshape(*dims) # of size [2, 4]
    stds = torch.abs(torch.randn(*dims))*3 # of size [2, 4]

    # Make sure with and without event dim we get the same result
    mixture = distr.Categorical(probs=mixture_probs) # of size [2, 4] but 4 is the number of components
    ...
    components2 = StableNormal(loc=locs.reshape(locs.shape + (1,)*extra_ndims), 
                               scale=stds.reshape(stds.shape + (1,)*extra_ndims)) # batch_shape of [2,4,1,1]
    components2 = distr.Independent(components2, extra_ndims) # batch_shape of [2,4] event_shape of [1,1]
